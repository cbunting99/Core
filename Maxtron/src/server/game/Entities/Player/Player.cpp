...
{
    TC_LOG_DEBUG(LOG_FILTER_GENERAL, "BattleGroundQueue::CheckCrossFactionMatch");
    ...
    {
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Not enough players. Has: %u Need: %u", m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() + m_SelectionPools[TEAM_HORDE].GetPlayerCount(), MPT * 2);
        return false;
    }
    return true;
}
...
bool BattlegroundQueue::FillXPlayersToBG(BattlegroundBracketId bracket_id, Battleground* bg, bool start)
{
    if (sWorld->getBoolConfig(CONFIG_CROSSFACTIONBG))
    {
        ...
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, "valiFree: %u vhordeFree: %u", valiFree, vhordeFree);
        ...
        for (GroupsQueueType::iterator itr = m_QueuedGroups[bracket_id][BG_QUEUE_MIXED].begin(); itr != m_QueuedGroups[bracket_id][BG_QUEUE_MIXED].end(); ++itr)
        {
            ...
            TC_LOG_DEBUG(LOG_FILTER_GENERAL, "moreAli: %s", moreAli ? "true" : "false");
            TC_LOG_DEBUG(LOG_FILTER_GENERAL, "diff: %u", diff);
            ...
            TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Team: %s", alliance ? "ali" : "horde");
            ...
            if (m_SelectionPools[alliance ? TEAM_ALLIANCE : TEAM_HORDE].AddGroup((*itr), alliance ? aliFree : hordeFree))
            {
                ...
                TC_LOG_DEBUG(LOG_FILTER_GENERAL, "Size: %u", GSize);
