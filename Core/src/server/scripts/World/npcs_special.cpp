...
UPDATE `creature_template` SET `flags_extra` = '128',
`ScriptName` = 'npc_power_word_barrier' WHERE `creature_template`.`entry` =33248;*/
		...
		void BarrierChecker(Unit *who) {
			...
			if (who->isAlive() && who->HasAura(81782)) {
				if (AuraEffect const* aur = who->GetAuraEffect(81782, 0))
					aur->GetBase()->SetDuration(aur->GetSpellInfo()->GetDuration());
			}
		}
